import { Plugin, Panel, AuthUser, AuthHookContext, KratosJsRequest, KratosJsResponse, t } from '@maxal_studio/kratosjs';
import en from './lang/en';
import sq from './lang/sq';
import type { EntityManager, EntitySchema } from '@mikro-orm/core';
import { createUserTwoFactorEntity, type IUserTwoFactor } from './entities/UserTwoFactor';
import { Migration20260101000000CreateUserTwoFactor } from './migrations/sql/Migration20260101000000CreateUserTwoFactor';
import { generateSecret, verifyToken, buildOtpAuthUrl, buildQrDataUrl } from './totp';
...
export class TwoFactorPlugin extends Plugin {
	...
	register(panel: Panel): void {
		...
		panel.registerAuthChallenge({
			...
			verify: async (user: AuthUser, payload: unknown, ctx: AuthHookContext) => {
				const record = await this.findRecord(ctx.getEm(), user.id);
		...
		panel.registerRoute('get', '/auth/2fa/status', (req, res) => this.handleStatus(req, res));
		panel.registerRoute('post', '/auth/2fa/setup', (req, res) => this.handleSetup(req, res));
		panel.registerRoute('post', '/auth/2fa/enable', (req, res) => this.handleEnable(req, res));
		panel.registerRoute('post', '/auth/2fa/disable', (req, res) => this.handleDisable(req, res));
	}

	/**
	 * GET /auth/2fa/status — whether the current user has 2FA enabled. Drives the settings UI.
	 */
	private async handleStatus(req: KratosJsRequest, res: KratosJsResponse): Promise<void> {
	...
	private findRecord(em: EntityManager, userId: string): Promise<IUserTwoFactor | null> {
	 ...
	 */
	private async handleSetup(req: KratosJsRequest, res: KratosJsResponse): Promise<void> {
		const user = req.authUser;
		if (!user) {
			res.status(401).json({ error: t('2fa:error.auth_required') });
			return;
	 ...
	 */
	private async handleEnable(req: KratosJsRequest, res: KratosJsResponse): Promise<void> {
		const user = req.authUser;
		if (!user) {
			res.status(401).json({ error: t('2fa:error.auth_required') });
			return;
	 ...
	 */
	private async handleDisable(req: KratosJsRequest, res: KratosJsResponse): Promise<void> {
		const user = req.authUser;
		if (!user) {
			res.status(401).json({ error: t('2fa:error.auth_required') });
			return;
